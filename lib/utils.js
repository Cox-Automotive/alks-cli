...
exports.isPowerShell = function(cb){
    if(!exports.isWindows()) return cb(false);

    var cmd = '(dir 2>&1 *`|echo CMD);&<# rem #>echo PowerShell';
    exec(cmd, function(err, stdout, stderr){
        if(err || stderr.length) return cb(false);
        else{
            return exports.trim(stdout) === 'PowerShell';
        }
    });
};
