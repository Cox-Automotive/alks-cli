...
exports.checkForUpdate = function(){
    ...
    client.get('https://registry.npmjs.org/alks', { timeout: 1000 }, function (error, data, raw, res){
        if(!error){
            var latestVer   = data['dist-tags'].latest,
                needsUpdate = semVer.gt(latestVer, myVer);

            console.log('Latest version of ALKS CLI: %s and you have %s. need update? %s', latestVer, myVer, needsUpdate);
        }
    });
};