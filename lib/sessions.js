...
exports.getSessionKey = function(program, logger, alksAccount, alksRole, iamOnly, forceNewSession, filterFavorites, cb){
    async.waterfall([
        ...
        function(developer, auth, alksAccount, alksRole, callback){
            ...
            keys.getKeys(auth, false, function(err, keys){
                callback(null, developer, auth, alksAccount, alksRole, keys);
            });
        },
        ...
        function(developer, auth, alksAccount, alksRole, existingKeys, callback){
            ...
            alks.getAccessToken({
                ...
                // Get durations
                alksCreate.getLoginRole({
                    accountId: data.alksAccount,
                    role: data.alksRole
                }).then((durations) => {

                    let duration = Math.min(_.last(durations), 36);
                    ...
                    alksCreate.getKeys({
                        ...
                        keys.addKey(key.accessKey, key.secretKey, key.sessionToken,
                            data.alksAccount, data.alksRole, data.expires, auth, false);

                    }).catch((error) => {
                        utils.log(program, logger, 'error generating keys/session. Please check API URL. Error: ' + error);
                        new Error(utils.getBadAccountMessage());
                    });

                }).catch(error => {
                    utils.log(program, logger, 'error retrieving duration: ' + error);
