...
exports.getSessionKey = function(program, logger, alksAccount, alksRole, iamOnly, forceNewSession, filterFavorites, cb){
    async.waterfall([
        ...
        function(developer, auth, alksAccount, alksRole, existingKeys, callback){
            ...
            alks.getDurations(data, auth, { debug: program.verbose, ua: utils.getUA() }, function(err, durations){
                if(err) return callback(err);

                var duration = Math.min(_.last(durations), 36);
                ...
                alks.createKey(data, auth, duration, { debug: program.verbose, ua: utils.getUA() }, function(err, key){
                    if(!err){
