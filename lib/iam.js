...
exports.getIAMKey = function(program, logger, alksAccount, alksRole, forceNewSession, filterFavorites, cb){
    async.waterfall([
        ...
        function(developer, auth, alksAccount, alksRole, existingKeys, callback){
            ...
            (async function() {
                let alks = ALKS.create({
                    baseUrl: developer.server.slice(0, developer.server.length - 1),
                ...
                alksNode.createIamKey(data, auth, duration, { debug: program.verbose, ua: utils.getUA() }, function(err, key){
                    if(!err){
