...
exports.getALKSAccount = function(program, options, callback){
    ...
    async.waterfall([
        ...
        function(developer, password, cb){
            alks.getAccounts(developer.server, developer.userid, password, { filter: { iamKeyActive: opts.iamOnly } }, function(err, alksAccounts){
        ...
        function(developer, password, alksAccounts, cb){
            if(opts.selectIndex !== null){
                if(opts.selectIndex <= alksAccounts.length-1){
                    var data = alksAccounts[opts.selectIndex].substring(4).split(alks.getAccountSelectorDelimiter()),
                        alksAccount = data[0],
                        alksRole = data[1];
