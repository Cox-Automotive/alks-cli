...
var getPasswordFromKeystore = exports.getPasswordFromKeystore = function(cb){
...
var removePassword = exports.removePassword = function(){
    if(utils.isPasswordSecurelyStorable()){
...
var getPasswordFromPrompt = exports.getPasswordFromPrompt = function (callback, text, currentPassword){
    prompt.start();
    prompt.message = '';
    ...
    prompt.get([{
        ...
        hidden: true,
        replace: '*',
        default: _.isEmpty(currentPassword) ? '' : currentPassword,
        required: true
    }], function(err, result){
        if(err){
            callback(err);
...
exports.getALKSAccount = function(program, options, callback){
    ...
    async.waterfall([
        ...
        function(developer, password, cb){
            alks.getAccounts(opts.server, opts.userid, password, { filters: { iamOnly: opts.iamOnly } }, function(err, alksAccounts){
        ...
        function(developer, password, alksAccounts, cb){
            ...
            inquirer.prompt([ promptData ]).then(function(answers){
                var acctStr     = answers.alksAccount,
                    data        = acctStr.substring(4).split(alks.getAccountSelectorDelimiter()),
                    alksAccount = data[0],
                    alksRole    = data[1];

                developer.lastAcctUsed = acctStr; // remember what account they last used

                saveDeveloper(developer, function(err, savedData){
