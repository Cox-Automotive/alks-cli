...
exports.getPasswordFromPrompt = async function getPasswordFromPrompt(text, currentPassword) {
...
exports.getALKSAccount = async function getALKSAccount(program, options) {
    ...
    const indexedAlksAccounts = alksAccounts
        ...
        .filter(alksAccount => !opts.filterFavorites || favorites.contains(alksAccount)) // Filter out non-favorites if filterFavorites flag is passed
        .sort((a, b) => favorites.includes(b) - favorites.includes(a)) // Move favorites to the front of the list, non-favorites to the back
        .map(alksAccount => [alksAccount.account, alksAccount.role].join(exports.getAccountDelim()));
...
exports.getPassword = async function getPassword(program) {
    if (program && !_.isEmpty(program.password)) { // first check password from CLI argument
        utils.log(program, logger, 'using password from CLI arg');
        return program.password;
    } else if (!_.isEmpty(process.env.ALKS_PASSWORD)) { // then check for an environment variable
        utils.log(program, logger, 'using password from environment variable');
        return process.env.ALKS_PASSWORD;
    } else { // then check the keystore
