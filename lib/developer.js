        ...
        if(_.isEmpty(developer.server)  ||
           ...
           _.isEmpty(developer.alksRole)){
            ...
            if(semVer.gt(currentVersion, lastRunVerion)){
                ...
                // TODO: pull release notes from NPM
                var msg = [
                    'Thanks for upgrading to version ',
                    currentVersion,
                    ' lorem ipsum dolimar lorem ipsum dolimar!!\n\nHere be some release notes..'
                ].join('');
                utils.showBorderedMessage(110, clc.white(msg));
...
var saveDeveloper = exports.saveDeveloper = function(data, callback){console.error('sabin');
...
exports.getALKSAccount = function(program, selectIndex, callback){
    async.waterfall([
        // get developer
        function(cb){
            getDeveloper(cb);
        ...
        function(developer, password, cb){
            alks.getAccounts(developer.server, developer.userid, password, function(err, alksAccounts){
        ...
        function(developer, password, alksAccounts, cb){
            if(selectIndex !== null){
                if(selectIndex <= alksAccounts.length-1){
                    var data = alksAccounts[selectIndex].substring(4).split(alks.getAccountSelectorDelimiter()),
                        alksAccount = data[0],
                        alksRole = data[1];
                    ...
                    return callback(new Error('Provided alks account index out of bounds! Use 1-' + alksAccounts.length));
                }
            }
            ...
            inquirer.prompt([
                ...
                developer.account = alksAccount;
                developer.role = alksRole;
