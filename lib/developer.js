...
exports.getALKSAccount = function(program, options, callback){
    ...
    async.waterfall([
        ...
        function(developer, password, alksAccounts, cb){
            if(opts.filterFavorites !== true) return cb(null, developer, password, alksAccounts);

            exports.getFavorites(function(err, faves){
                if(err) return cb(err);

                alksAccounts = _.filter(alksAccounts, function(acct){
                    return _.contains(faves.favorites, acct);
                });
