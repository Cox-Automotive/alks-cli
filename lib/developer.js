    ...
    async.waterfall([
        ...
        function(developer, auth, cb){
            alks.getAccessToken({
                ...
                alksCreate.getAccounts().then(alksAccount => {
                    alksAccount.forEach((alksAccount) => {
                       if (opts.iamOnly === true && alksAccount.iamKeyActive === false) return ;

                       alksAccount = [alksAccount.account, alksAccount.role].join(exports.getAccountDelim());
                       indexedAlksAccounts.push(alksAccount);
                    });
                    cb(null, developer, auth, indexedAlksAccounts);
                }).catch(error => {
                    utils.log(program, logger, 'error getting accounts: ' + error);
                    cb(error, developer, auth, null);
                });
            }).catch(error => {
