...
exports.getKeyOutput = function(format, key, profile, force, cb){
    ...
    if(format === 'docker'){
        ...
        var isWin = utils.isWindows();

        if(!isWin){
            return cb([
                'export AWS_ACCESS_KEY_ID=', key.accessKey, ' && ',
                'export AWS_SECRET_ACCESS_KEY=', key.secretKey, ' && ',
                'export AWS_SESSION_TOKEN=', key.sessionToken,  ' && ',
                'export AWS_SESSION_EXPIRES=', keyExpires
            ].join(''));
        }
        else{
            utils.isPowerShell(function(isPowerShell){
                if(!isPowerShell){
                    return cb([
                        'SET AWS_ACCESS_KEY_ID=', key.accessKey, ' && ',
                        'SET AWS_SECRET_ACCESS_KEY=', key.secretKey, ' && ',
                        'SET AWS_SESSION_TOKEN=', key.sessionToken,  ' && ',
                        'SET AWS_SESSION_EXPIRES=', keyExpires
                    ].join(''));
                }
                else{
                    return cb([
        ...
        if(updateCreds(key, profile, force)){
            ...
            return cb(clc.red('The ' + profile + ' profile already exists in AWS credentials. Please pass -f to force overwrite.'));
        }
    }
    else if(format === 'idea'){
        return cb([
        ...
        return cb(JSON.stringify(key, null, 4));
    }
};