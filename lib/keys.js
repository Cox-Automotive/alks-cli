...
exports.getKeyOutput = function(format, key, profile, force){
    ...
    else if(format === 'terraformenv'){
        var cmd = utils.isWindows() ? 'SET' : 'export';

        return [
            cmd, ' ALKS_ACCESS_KEY_ID=', key.accessKey, ' && ',
            cmd, ' ALKS_SECRET_ACCESS_KEY=', key.secretKey, ' && ',
            cmd, ' ALKS_SESSION_TOKEN=', key.sessionToken, ' && ',
            cmd, ' ALKS_SESSION_EXPIRES=', keyExpires
        ].join('');
    }
    else if(format === 'json'){
