...
function encrypt(text, key) {
    ...
    const cipher = crypto.createCipheriv(ALGORITHM, Buffer.from(getSizedEncryptionKey(key)), iv);
...
function decrypt(text, key) {
    ...
    const decipher = crypto.createDecipheriv(ALGORITHM, Buffer.from(getSizedEncryptionKey(key)), iv);
...
async function getKeysCollection() {
    return new Promise((resolve, reject) => {
        ...
        db.loadDatabase({}, (err) => {
            ...
            const keys = db.getCollection('keys') || db.addCollection('keys', { indices: ['expires'] });
...
exports.addKey = async function addKey(accessKey, secretKey, sessionToken, alksAccount, alksRole, expires, auth, isIAM) {
