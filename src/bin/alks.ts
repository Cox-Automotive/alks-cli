...
const alks = new Command();

alks.exitOverride();

alks.configureOutput({
  writeErr: (str) => {
...
alks.version(version).option('-v, --verbose', 'be verbose');

const sessions = alks.command('sessions').description('manage aws sessions');
...
const iam = alks.command('iam').description('manage iam resources');
...
const server = alks.command('server').description('ec23 metadata server');
